...
interface CheckboxQuestionProps {
  ...
  initialData?: CheckboxQuestionData;
}
...
export default function CheckboxQuestion({
  ...
  const [maxSelections, setMaxSelections] = useState<number>(
    initialData?.max_selections || 1
  );
  const [minSelections, setMinSelections] = useState<number>(
    initialData?.min_selections || 1
  );

  // Options for checkbox question - can be expanded
          ...
          <div className="flex justify-between items-center">
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                setMinSelections(Math.max(1, Math.min(value, options.length)));
              }}
              className="w-16 h-8 text-right"
              placeholder="1"
            />
